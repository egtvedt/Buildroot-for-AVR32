...
#
#############################################################
U_BOOT_TOOLS_VER:=1.2.0
U_BOOT_TOOLS_SOURCE:=u-boot-$(U_BOOT_TOOLS_VER).tar.bz2
U_BOOT_TOOLS_SITE:=ftp://ftp.denx.de/pub/u-boot
U_BOOT_TOOLS_DIR:=$(BUILD_DIR)/u-boot-$(U_BOOT_TOOLS_VER)
U_BOOT_TOOLS_CAT:=$(BZCAT)
U_BOOT_TOOLS_BIN:=tools/mkimage
...
$(U_BOOT_TOOLS_DIR)/.unpacked: $(DL_DIR)/$(U_BOOT_TOOLS_SOURCE)
	$(U_BOOT_TOOLS_CAT) $(DL_DIR)/$(U_BOOT_TOOLS_SOURCE) | tar -C $(BUILD_DIR) $(TAR_OPTIONS) -
	toolchain/patch-kernel.sh $(U_BOOT_TOOLS_DIR) target/bootloader/u-boot/ u-boot-$(U_BOOT_TOOLS_VER)\*.patch\*
	@touch $@
...
$(U_BOOT_TOOLS_DIR)/$(U_BOOT_TOOLS_BIN): $(U_BOOT_TOOLS_DIR)/.configured
	$(TARGET_CONFIGURE_OPTS) \
		CFLAGS="$(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" \
		$(MAKE) -C $(U_BOOT_TOOLS_DIR)
	cp -dpf $(U_BOOT_TOOLS_DIR)/$(U_BOOT_TOOLS_BIN) $(STAGING_DIR)/usr/bin/

u-boot-tools: $(U_BOOT_TOOLS_DIR)/$(U_BOOT_TOOLS_BIN)
